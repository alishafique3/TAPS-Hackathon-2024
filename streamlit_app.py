...
output_fp = r"datasets\plots_boundary1_with_NDVI.shp"
...
##############
    # Dataset selection
    dataset_option = st.selectbox("Select Dataset", ["Nitrogen Fertilizer Application", "Irrigation Applied", "Electrical Conductivity and pH", "Volumetric Water Content", "Weather and Climate", "NDVI_Ceres", "Crop Water Requirement Analysis"], key="dataset_select")

    # Parameter selection based on dataset choice
    if dataset_option == "Electrical Conductivity and pH":
        parameter_option = st.selectbox("Select Parameter", ["EC shallow", "EC deep", "pH"], key="ec_ph_select")
        gdf = gdf_pb_ss2
    
    elif dataset_option == "Nitrogen Fertilizer Application":
        parameter_option = st.selectbox("Select Date", nitrogen_date_columns, key="nitrogen_date_select")
        gdf = gdf1
    
    elif dataset_option == "Irrigation Applied":
        parameter_option = st.selectbox("Select Date", irrigation_date_columns, key="irrigation_date_select")
        gdf = gdf2
    
    elif dataset_option == "Volumetric Water Content":
        date_option = st.selectbox("Select Date Group", sorted(gdf4['Group_date'].dropna().unique()), key="vwc_date_select")
        depth_option = st.selectbox("Select Depth (inches)", depth_columns, key="vwc_depth_select")
        ...
        ], key="climate_param_select")
    
    elif dataset_option == "NDVI_Ceres":
        selected_date = st.selectbox("Select Date", list(ndvi_date_map.keys()), key="ndvi_date_select")
...
if dataset_option == "Electrical Conductivity and pH":
    if parameter_option == "EC shallow":
        ...
        # Function to download filtered EC Shallow data as CSV
        def download_ec_shallow_dataframe(df, label="Download EC Shallow Data"):
            csv = df.to_csv(index=False)
            st.download_button(
                label=label,
                data=csv,
                file_name="EC_shallow_filtered_data.csv",
                mime="text/csv"
            )
    
        # Display download button for EC Shallow data
        download_ec_shallow_dataframe(gdf[["TRT_ID", "EC shallow avg"]], label="Download Filtered EC Shallow Data")
        ...
        # Display filtered data for EC Shallow
        #st.markdown("**First 5 Rows of Filtered EC Shallow Data**")
        #st.dataframe(gdf[["TRT_ID", "EC shallow avg"]].head(5))
    ...
    # Display EC deep statistics in metrics format
        ...
        # Display filtered data for EC Deep
        st.markdown("**First 5 Rows of Filtered EC Deep Data**")
        st.dataframe(gdf[["TRT_ID", "EC deep avg"]].head(5))
        
        def download_ec_deep_dataframe(df, label="Download EC Deep Data"):
            csv = df.to_csv(index=False)
            st.download_button(
                label=label,
                data=csv,
                file_name="EC_deep_filtered_data.csv",
                mime="text/csv"
            )

        # Display download button for EC Deep data
        download_ec_deep_dataframe(gdf[["TRT_ID", "EC deep avg"]], label="Download Filtered EC Deep Data")
    ...
    # Display pH statistics in metrics format
        ...
        # Display filtered data for pH
        st.markdown("**First 5 Rows of Filtered pH Data**")
        st.dataframe(gdf[["TRT_ID", "pH avg"]].head(5))
        
        # Function to download filtered pH data as CSV
        def download_ph_dataframe(df, label="Download pH Data"):
            csv = df.to_csv(index=False)
            st.download_button(
                label=label,
                data=csv,
                file_name="pH_filtered_data.csv",
                mime="text/csv"
            )
    
        # Display download button for pH data
        download_ph_dataframe(gdf[["TRT_ID", "pH avg"]], label="Download Filtered pH Data")
    ...
    # Function to download filtered nitrogen data as CSV
    def download_nitrogen_dataframe(df, label="Download Nitrogen Data"):
        csv = df.to_csv(index=False)
        st.download_button(
            label=label,
            data=csv,
            file_name="Nitrogen_filtered_data.csv",
            mime="text/csv"
        )

    # Display download button
    download_nitrogen_dataframe(gdf[['TRT_ID', parameter_option]], label="Download Filtered Nitrogen Data")
    ...
    # Display filtered data for the selected date
    #st.markdown("**First 5 Rows of Filtered Nitrogen Fertilizer Application Data**")
    #st.dataframe(gdf.iloc[:5, [gdf.columns.get_loc('TRT_ID'), gdf.columns.get_loc(parameter_option)]])
    ...
    display_spatial_and_box_plots(
        gdf,
        avg_col=parameter_option,
        title=f"Irrigation Application on {str(parameter_option)[0:10]}",
        cbar_label="Irrigation Amount (inches)",
        cmap_choice=cm.Blues
    )
    ...
    # Function to download filtered irrigation data as CSV
    def download_irrigation_dataframe(df, label="Download Irrigation Data"):
        csv = df.to_csv(index=False)
        st.download_button(
            label=label,
            data=csv,
            file_name="Irrigation_filtered_data.csv",
            mime="text/csv"
        )

    # Display download button
    download_irrigation_dataframe(gdf[['TRT_ID', parameter_option]], label="Download Filtered Irrigation Data")
    ...
    # Display filtered data for the selected date
    #st.markdown("**First 5 Rows of Filtered Irrigation Data**")
    #st.dataframe(gdf.iloc[:5, [gdf.columns.get_loc('TRT_ID'), gdf.columns.get_loc(parameter_option)]])
...
####################
    ...
    # Function to download filtered VWC data as CSV
    def download_vwc_dataframe(df, label="Download VWC Data"):
        csv = df.to_csv(index=False)
        st.download_button(
            label=label,
            data=csv,
            file_name="VWC_filtered_data.csv",
            mime="text/csv"
        )

    # Display download button
    download_vwc_dataframe(filtered_data[[depth_option, 'Group_date', 'TRT_ID']], label="Download Filtered VWC Data")
    ...
    # Display filtered data for the selected date and depth
    #st.markdown("**First 5 Rows of Filtered Volumetric Water Content Data**")
    #st.dataframe(filtered_data.iloc[:5, [gdf4.columns.get_loc('TRT_ID'), gdf4.columns.get_loc('Group_date'), gdf4.columns.get_loc(depth_option)]])
...
if dataset_option == "Weather and Climate":
    st.markdown(
        f"<h3 style='text-align: center;'>Weather and Climate</h3>",
        unsafe_allow_html=True
    )
    if climate_parameter_option == "Annual Precipitation":
        ...
        response = get_chatgpt_response(
            f"Please provide brief data analysis about Weather and Climate in the context of sustainable irrigation and data: {annual_precipitation}."
        )
        
        # Display ChatGPT's response in the description box
        display_description_box(
            " 🤖 Analysis of Weather and Climate by A.I. WildCat",
            response
        )
        ...
        # Generate the A.I. WildCat prompt with selected date, max, min, and mean values
        response = get_chatgpt_response(
            f"Please provide brief data analysis about Weather and Climate in the context of sustainable irrigation and data: {annual_eto}."
        )
        ...
        # Generate the A.I. WildCat prompt with selected date, max, min, and mean values
        response = get_chatgpt_response(
            f"Please provide brief data analysis about Weather and Climate in the context of sustainable irrigation and data1: {annual_tmmn} and data2: {annual_tmmx}."
        )
        ...
        # Generate the A.I. WildCat prompt with selected date, max, min, and mean values
        response = get_chatgpt_response(
            f"Please provide brief data analysis about Weather and Climate in the context of sustainable irrigation and data1: {annual_rmin} and data2: {annual_rmax}."
        )
        ...
        # Generate the A.I. WildCat prompt with selected date, max, min, and mean values
        response = get_chatgpt_response(
            f"Please provide brief data analysis about Weather and Climate in the context of sustainable irrigation and data: {annual_vs}."
        )
        ...
        # Generate the A.I. WildCat prompt with selected date, max, min, and mean values
        response = get_chatgpt_response(
            f"Please provide brief data analysis about Weather and Climate in the context of sustainable irrigation and data: {annual_srad}."
        )
...
if dataset_option == "NDVI_Ceres" and selected_date:
   ...
   # Calculate max, min, and mean NDVI values for the selected date column
    ...
    filtered_ndvi_data = plots_boundary1[['TRT_ID', 'Hybrid', ndvi_date_option]]
    

    # Function to download filtered NDVI data as CSV
    def download_ndvi_dataframe(df, label="Download NDVI Data"):
        csv = df.to_csv(index=False)
        st.download_button(
            label=label,
            data=csv,
            file_name="NDVI_filtered_data.csv",
            mime="text/csv"
        )

    # Display download button for NDVI data
    download_ndvi_dataframe(filtered_ndvi_data, label="Download Filtered NDVI Data")
    ...
    # Display filtered data for the selected NDVI date
    #st.markdown("**First 5 Rows of Filtered NDVI Data**")
    #st.dataframe(filtered_ndvi_data.head(5))
...
# Function to allow downloading of the Crop Water Requirement dataframe
def download_dataframe(df, label="Download CSV"):
    csv = df.to_csv(index=False)
    st.download_button(
        label=label,
        data=csv,
...
# Define parameters for selection in Crop Water Requirement Analysis
cwr_columns = CWR1.columns[3:]  # Selecting columns from the third to the end

# Sidebar options for selecting first and second parameters in Crop Water Requirement Analysis
if dataset_option == "Crop Water Requirement Analysis":
    
    st.markdown(
        f"<h3 style='text-align: center;'>Crop Water Requirement Analysis</h3>",
        unsafe_allow_html=True
    )
    # Define parameters for selection in Crop Water Requirement Analysis
    cwr_columns = CWR1.columns[3:]  # Selecting columns from the third column to the end
    
    planting_date = st.selectbox("Planting Time", ["Early", "Mid", "Late"], key="cwr_planting_date_select")
    
    # Adding unique keys for selecting two parameters to plot in CWR analysis
    selected_cwr_parameter1 = st.selectbox("Select First Parameter for Trend Analysis", options=cwr_columns, key="cwr_param1_select")
    selected_cwr_parameter2 = st.selectbox("Select Second Parameter for Trend Analysis", options=cwr_columns, key="cwr_param2_select")
    
    
    
# Modify the plotting function to plot both selected parameters
def plot_selected_cwr_parameter_interactive(planting_data, planting_date, parameter1, parameter2):
    fig = go.Figure()

    # Plot the first selected parameter
    fig.add_trace(go.Scatter(
        x=planting_data['TIMESTAMP'],
        y=planting_data[parameter1],
        mode='lines+markers',
        name=parameter1,
        line=dict(color='blue')
    ))

    # Plot the second selected parameter
    fig.add_trace(go.Scatter(
        x=planting_data['TIMESTAMP'],
        y=planting_data[parameter2],
        mode='lines+markers',
        name=parameter2,
        line=dict(color='orange')
    ))

    # Define stage dates based on planting type
    if planting_date == "Early":
        initiation_start = "2024-05-08"
        initiation_end = "2024-06-07"
        mid_start = "2024-06-08"
        mid_end = "2024-09-05"
        end_start = "2024-09-06"
        end_end = "2024-10-15"
    elif planting_date == "Mid":
        initiation_start = "2024-05-15"
        initiation_end = "2024-06-14"
        mid_start = "2024-06-15"
        mid_end = "2024-09-12"
        end_start = "2024-09-13"
        end_end = "2024-10-15"
    elif planting_date == "Late":
        initiation_start = "2024-05-31"
        initiation_end = "2024-06-30"
        mid_start = "2024-07-01"
        mid_end = "2024-09-20"
        end_start = "2024-09-21"
        end_end = "2024-10-15"

    # Add shaded regions for each stage
    fig.add_vrect(x0=initiation_start, x1=initiation_end,
                  fillcolor="blue", opacity=0.2, line_width=0,
                  annotation_text="Initiation Stage", annotation_position="top left")
    fig.add_vrect(x0=mid_start, x1=mid_end,
                  fillcolor="orange", opacity=0.2, line_width=0,
                  annotation_text="Mid Stage", annotation_position="top left")
    fig.add_vrect(x0=end_start, x1=end_end,
                  fillcolor="red", opacity=0.2, line_width=0,
                  annotation_text="End Stage", annotation_position="top left")

    # Update layout with title and labels
    fig.update_layout(
        title=f'{planting_date} Planting - {parameter1} and {parameter2} Trends',
        xaxis_title='Date',
        yaxis_title='Values',
        hovermode='x unified'
    )
...
# Display the selected parameter analysis only when "Crop Water Requirement Analysis" is selected
if dataset_option == "Crop Water Requirement Analysis":
    st.markdown(f"<h3 style='text-align: center;'>{selected_cwr_parameter1} and {selected_cwr_parameter2} Trends</h3>", unsafe_allow_html=True)

    # Filter data for the selected planting type
    planting_data = CWR1[CWR1['Planting'] == planting_date]

    # Call the modified plotting function
    plot_selected_cwr_parameter_interactive(planting_data, planting_date, selected_cwr_parameter1, selected_cwr_parameter2)
    
    # Display the first five rows of the dataframe
    st.markdown(f"**First 5 Rows of {planting_date} Planting Data**")
    st.dataframe(planting_data.iloc[:5,])  # Display the first five rows of the dataframe
    
    # Add download button for the dataframe
    download_dataframe(planting_data, label=f"Download {planting_date} Planting Data")
    
    # Generate the A.I. WildCat prompt with selected date, max, min, and mean values
    response = get_chatgpt_response(
        "Please provide brief insight about Crop Water Requirement in the context of sustainable irrigation."
    )
    
    # Display ChatGPT's response in the description box
    display_description_box(
        " 🤖 Analysis of Crop Water Requirement by A.I. WildCat",
        response
    )
